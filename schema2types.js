...
const url = process.env.OPENAPI_URL || 'http://127.0.0.1:8000/ispyb/api/v1/openapi.json';
    ...
    try {
      ...
      for (const schemaName in json.components.schemas) {
        ...
        sc2t
          .compile(schema)
          .then((ts) => {
            const construct = 'type Constructor<T = {}> = new (...args: any[]) => T;';
...
export function with${name}<TBase extends Constructor>(Base: TBase) {
  return class With${name} extends Base {\n  ${lines.slice(1).join('\n  ').slice(0, -2)}}`;
            ...
            fs.writeFileSync(`src/models/${schemaName}.d.ts`, ts + '\n' + construct + classes.join('') + '\n');
          })
          .catch((e) => {
