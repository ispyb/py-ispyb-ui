...
function getContainerOn(cell: number, position: number, changer: AbstractSampleChanger, containers: ContainerDewar[] | undefined, beamline: Beamline) {
  if (!containers) {
    return undefined;
  }
  ...
  if (changer) {
    const containerJSX = range(0, changer.getNbCell()).map((cell) => {
      return range(0, changer.getNbContainerInCell(cell)).map((position) => {
        const container = getContainerOn(cell, position, changer, containers, beamline);
        return (
          <ChangerContainer
            beamline={beamline}
...
function ChangerContainer({
  ...
  const { x, y, r, xtxt, ytxt } = changer.getContainerCoordinates(cell, position);
  ...
  return (
    <g>
      ...
      {container ? (
        <g>
          <circle cx={x} cy={y} r={r} stroke="black" fill="transparent"></circle>
          <RemoveContainerBtn
            onClick={() => {
            ...
            cr={r}
          ></RemoveContainerBtn>
          <InfoContainerBtn proposalName={proposalName} container={container} cx={x} cy={y} cr={r}></InfoContainerBtn>
        </g>
      ) : (
        <DroppablePosition
