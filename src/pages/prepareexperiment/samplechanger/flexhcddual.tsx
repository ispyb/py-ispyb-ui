...
export class FlexHCDDual extends AbstractSampleChanger {
          ...
          return (
            <g>
              <circle
                ...
                fill="#FFFF"
              ></circle>
              <text className="cellNumber" x={this.sampleChangerRadius * Math.sin(angle)} y={-this.sampleChangerRadius * Math.cos(angle) + 3}>
                {n + 1}
              </text>
            </g>
          );
  ...
  getContainerCoordinates(cell: number, position: number): { x: number; y: number; r: number; xtxt: number; ytxt: number } {
    ...
    return { x: x[position], y: y[position], r, xtxt: xtxt[position], ytxt: ytxt[position] };
  }
  ...
  getContainerType(cell: number, position: number): containerType {
    const types: containerType[] = ['Spinepuck', 'Unipuck', 'Spinepuck', 'Unipuck', 'Unipuck', 'Unipuck', 'Unipuck', 'Unipuck'];
