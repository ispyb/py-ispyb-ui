...
export default function ProposalTable({ data }: { data: Proposal[] }) {
  const columns: ColumnDescription[] = [
    ...
    },
    {
      ...
      sort: true,
      sortValue: (cell, row) => `${row.Proposal_proposalCode}${row.Proposal_proposalNumber}`,
      filterValue: (cell, row) => `${row.Proposal_proposalCode}${row.Proposal_proposalNumber}`,
  ...
  return (
    <>
      <p style={{ fontSize: 15, fontWeight: 'bold', margin: 'auto' }}>
        <Badge bg="secondary" style={{ marginRight: 5 }}>{`${proposal.Proposal_proposalType}`}</Badge>
