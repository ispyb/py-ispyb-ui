...
import { useShippingHistory } from 'hooks/ispyb';
...
export function TransportPane({ shipping, proposalName }: { shipping: Shipping; proposalName: string }) {
  const { data, isError } = useShippingHistory({ proposalName, shippingId: shipping.shippingId });
    ...
    return (
      <Row style={{ margin: 10 }}>
        <Alert variant="warning">
          <FontAwesomeIcon style={{ marginRight: 10 }} icon={faQuestionCircle}></FontAwesomeIcon>Could not find history for selected shipping
        </Alert>
      </Row>
    );
  }
...
export function DewarTransport({ history }: { history: DewarTransportHistory }) {
  return (
          ...
          <div className="historyParameterTable">
            <SimpleParameterTable
              parameters={[
                ...
                { key: 'Outbound tracking number', value: history.Dewar_trackingNumberToSynchrotron },
                { key: 'Return courier', value: history.Shipping_returnCourier },
                { key: 'Return tracking number', value: history.Dewar_trackingNumberFromSynchrotron },
              ]}
            ></SimpleParameterTable>
          </div>
        </Col>
